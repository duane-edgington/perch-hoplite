...
**July 9 2026: Full pipeline rebuild with per-window amplitude normalization fix.**
All embeddings regenerated. Old v1–v8 classifiers retired — trained on un-normalized
embeddings. New versioning starts at v0.

| Version | ROC-AUC | Training DB | humpback labels | Notes |
|---|---|---|---|---|
| **v0** | **0.9773** | April 2018 (normalized) | 41 | Baseline — April only, normalized embeddings |
| **v1** | **0.9533** | April 2018 + Oct 2020 (normalized) | 250 | Combined cross-season, normalized |
| **v2** | **0.9654** | April 2018 (normalized) | 41 | April only + expanded orca/dolphin/other labels |

v2 adds 20 new expert-reviewed labels (5 orca, 9 dolphin, 6 other) from April 20 2018
annotation session. Improved cmap (0.8810→0.8930) reflects better multi-class separation.
v1 remains the cross-season classifier; v2 is the April-only high-precision classifier.
...
Dr. John Ryan (MBARI) confirmed all April 30 humpback_song annotations
and added additional labels via the Gradio annotation interface — 41 total
expert-confirmed humpback clips. This is the first expert-validated humpback
training set for MARS hydrophone data.
...
**Round 2 — v8_clean, Oct 5/6/7/12 orca detections (201 clips, 4 batches of 25):**
...
Across three rounds of expert review totaling 100+ clips, no orca calls
were confirmed in the MARS hydrophone record despite documented CA140B
("Louise") and CA51A pod visits October 3-12. This is consistent with
known Bigg's (transient) orca behavior — transient orca are acoustically
quiet during active hunts, reserving calls for socializing.
...
positives across all classifier versions. The October expert review produced
209 confirmed humpback labels which will be used for v9_clean training.

**Total October 2020 expert-confirmed labels to date:**
- humpback_song: 209 (rounds 1-3 combined)
- dolphin_call: 5
- orca_call: 0
