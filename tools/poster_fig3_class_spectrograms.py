...
the hoplite DB's `annotations.offsets` blob, which is 2 little-endian doubles = (start, end)):
    ...
    humpback_song  MARS_20201001_054822_resampled_32kHz.wav   155.0-160.0 s
...
EXEMPLARS = [
    ...
    ("humpback_song", "Humpback song", "2020/10", "MARS_20201001_054822_resampled_32kHz.wav", 155.0, 160.0),
...
def build_figure(rows, out_path, dpi=300, window_len=5.0, colormap=COLORMAP, log_freq=False):
    ...
    for ax, (label, name, S_db, f, t) in zip(axes, rows):
        ...
        if ax is axes[0]:
            ax.set_ylabel("Frequency (Hz, mel)")
        else:
            ax.set_yticklabels([])
...
def main():
    ...
    if args.context_check:
        ...
        print(f"Wrote 30s context view: {out_path}")
        sys.exit(0)
