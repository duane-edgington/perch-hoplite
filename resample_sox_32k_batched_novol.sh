#!/usr/bin/env bash
#
# Resample PAM audio to 32 kHz (by default) with SoX.
#
# This variant does NOT apply the `vol` gain adjustment. The original 3x
# volume boost was a voltage calibration step, but it pushed peaks to full
# scale and produced "vol clipped" / "dither clipped" warnings. With no vol
# boost the samples stay in range and those warnings go away. Note this means
# the output amplitude is the raw resampled level (no 3x calibration applied) --
# use this version only if calibration is handled elsewhere or not needed.
#
# Caps how many SoX processes run at once (a concurrency limit) instead of
# launching every file into the background simultaneously, so the job behaves
# the same whether run locally or over a remote connection. Files are grouped
# by day for readable progress output.
#
# Sample rate:
#   Defaults to 32000 Hz. Override with the SAMPLE_RATE environment variable,
#   e.g.  SAMPLE_RATE=16000 ./resample_sox_32k_batched_novol.sh 2018 5
...
#
# Examples:
#   ./resample_sox_32k_batched_novol.sh 2018 5            # whole month, auto max_jobs
#   ./resample_sox_32k_batched_novol.sh 2018 5 2 2        # only May 2, 2018
#   ./resample_sox_32k_batched_novol.sh 2018 5 1 7        # May 1-7, 2018
#   ./resample_sox_32k_batched_novol.sh 2018 5 1 31 8     # whole month, 8 parallel jobs
#   SAMPLE_RATE=16000 ./resample_sox_32k_batched_novol.sh 2018 5   # 16 kHz instead
#
...
# short log fades in/out. No `vol` adjustment in this variant.
convert_one() {
  local infile="$1" outfile="$2"
  if ! sox "${infile}" -b 16 "${outfile}" \
        rate -v "${sample_rate}" highpass 10 fade 0.1 -0 0.1; then
    echo "${infile}" >> "${fail_log}"
  fi
}

printf "Starting (no vol): %04d-%02d days %d-%d, rate %s, up to %d parallel job(s)\n" \
